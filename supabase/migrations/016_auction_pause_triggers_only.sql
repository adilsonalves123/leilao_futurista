"-- Rode APÓS adicionar manualmente: ALTER TYPE auction_status ADD VALUE 'paused';
-- Contém apenas políticas, triggers e funções (sem ALTER TYPE).

-- Buscas públicas: apenas leilões ativos ou encerrados (paused/cancelled ocultos)
...
-- Impede alteração de preço/prazo com lances e controla transições de status
...
-- Bloqueia lances quando leilão não está live (inclui paused)
