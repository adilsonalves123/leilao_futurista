-- Jarvis comprador: leilões ao vivo abaixo do mercado estimado (dados reais do app)
...
DECLARE
  ...
  v_in_transit INT := 0;
  v_live_com_mercado INT := 0;
  v_compensa INT := 0;
  v_atencao INT := 0;
  v_melhores JSONB := '[]'::jsonb;
...
BEGIN
  ...
  -- Leilões ao vivo com mercado estimado e oportunidades (mesma lógica do veredito do app)
  SELECT COUNT(*)::INT INTO v_live_com_mercado
  FROM public.auctions a
  WHERE a.status = 'live'
    AND COALESCE(a.estimated_market_cents, 0) > 0;

  SELECT
    COUNT(*) FILTER (
      WHERE GREATEST(
        0,
        ROUND((1 - a.current_price_cents::NUMERIC / a.estimated_market_cents::NUMERIC) * 100)
      ) >= 20
    )::INT,
    COUNT(*) FILTER (
      WHERE GREATEST(
  ...
  INTO v_compensa, v_atencao
  FROM public.auctions a
  WHERE a.status = 'live'
    AND COALESCE(a.estimated_market_cents, 0) > 0
    AND a.current_price_cents < a.estimated_market_cents;

  SELECT COALESCE(
    jsonb_agg(sub.row ORDER BY (sub.row->>'discount_pct')::INT DESC),
    '[]'::jsonb
  )
  INTO v_melhores
  FROM (
    SELECT jsonb_build_object(
      ...
      'market_cents', a.estimated_market_cents,
      'discount_pct', GREATEST(
        0,
        ROUND((1 - a.current_price_cents::NUMERIC / a.estimated_market_cents::NUMERIC) * 100)
      )::INT,
      'verdict', CASE
        WHEN ROUND((1 - a.current_price_cents::NUMERIC / a.estimated_market_cents::NUMERIC) * 100) >= 20
          THEN 'good'
        WHEN ROUND((1 - a.current_price_cents::NUMERIC / a.estimated_market_cents::NUMERIC) * 100) >= 5
          THEN 'fair'
        ELSE 'bad'
      END,
    ...
    WHERE a.status = 'live'
      AND COALESCE(a.estimated_market_cents, 0) > 0
      AND a.current_price_cents <= (a.estimated_market_cents * 95 / 100)
    ORDER BY GREATEST(
      0,
      ROUND((1 - a.current_price_cents::NUMERIC / a.estimated_market_cents::NUMERIC) * 100)
    ) DESC
  ...
  IF v_compensa > 0 THEN
    v_alertas := v_alertas || jsonb_build_array(jsonb_build_object(
      'kind', 'market_deal',
      'severity', 'info',
      'title', format('%s lote(s) com veredito Compensa agora', v_compensa),
      'detail', 'Lance atual ≥20% abaixo do mercado estimado cadastrado.',
      'action_url', '/(tabs)/leiloes'
    ));
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'leiloes_oportunidades', jsonb_build_object(
      'live_com_mercado_estimado', v_live_com_mercado,
      'abaixo_mercado_compensa', v_compensa,
      'abaixo_mercado_atencao', v_atencao,
      'melhores', v_melhores
    ),
    'alertas', v_alertas
  );
